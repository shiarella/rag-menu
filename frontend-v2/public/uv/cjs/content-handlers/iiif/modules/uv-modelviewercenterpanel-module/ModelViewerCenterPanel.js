...
var ModelViewerCenterPanel = /** @class */ (function (_super) {
    ...
    ModelViewerCenterPanel.prototype.create = function () {
        ...
        this.$modelviewer = $("<model-viewer \n        ".concat(this.config.options.autoRotateEnabled ? "auto-rotate" : "", " \n        ").concat(this.config.options.interactionPromptEnabled
        ...
        this.$modelviewer[0].addEventListener("dblclick", function (e) {
            if (_this.config.options.doubleClickAnnotationEnabled) {
                ...
                _this.extensionHost.publish(Events_1.ModelViewerExtensionEvents.DOUBLECLICK, {
                    target: "".concat(canvas.id, "#xyz=").concat(point.position.x, ",").concat(point.position.y, ",").concat(point.position.z, "&nxyz=").concat(point.normal.x, ",").concat(point.normal.y, ",").concat(point.normal.z),
                });
            }
        });
    };
    ...
    ModelViewerCenterPanel.prototype.overlayAnnotations = function () {
        ...
        var annotationGroups = this.extension.annotations;
        annotationGroups.forEach(function (annoGroup) {
            annoGroup.points3D.forEach(function (point, index) {
                ...
                div.setAttribute("slot", "hotspot-".concat(index));
                div.setAttribute("data-position", "".concat(point.x, " ").concat(point.y, " ").concat(point.z));
                div.setAttribute("data-normal", "".concat(point.nx, " ").concat(point.ny, " ").concat(point.nz));
